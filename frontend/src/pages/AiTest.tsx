    ...
    if (!canRunCurrentMode) {
      const message = agentMode
        ...
        : providerStatus[provider] === 'unknown'
          ? 'เชื่อมต่อ API ไม่ได้ หรือระบบไม่ตอบกลับ'
        ...
        <div className="px-6 h-[70px] border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center">
          <div className="flex w-full items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              {(activeConversationId !== null || chatMessages.length >= 2) && (
                <button
                  type="button"
          ...
          <div className={isChatExpanded ? 'h-full' : 'space-y-6'}>
            ...
            {!isChatExpanded && loadError && (
              <div className="rounded-xl border border-rose-100 bg-rose-50 p-4 text-sm text-rose-700">
                {loadError} (เช็คว่า Supabase Edge Function พร้อมใช้งาน)
              </div>
            )}
                        ...
                        <div className="rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs text-amber-800">
                          {agentMode
                            ? (!hasKey.gemini
                              ...
                              : 'พร้อมใช้งาน')
                            : (providerStatus[provider] === 'unknown'
                              ? 'เชื่อมต่อ API ไม่ได้ (เช็ค Supabase Edge Function)'
                  ...
                  <div className={`p-5 space-y-4 ${isChatExpanded || chatMessages.length === 0 ? 'h-full flex flex-col min-h-0' : ''}`}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        title={isChatExpanded ? 'ย่อกลับขนาดเดิม' : 'ขยายเต็มจอ'}
                      >
                        <i className={`bi ${isChatExpanded ? 'bi-fullscreen-exit' : 'bi-arrows-fullscreen'}`}></i>
                        <span>{isChatExpanded ? 'ย่อกลับ' : 'ขยาย'}</span>
                      </button>
                    </div>
